...
void RandomGaussianBlur(const cv::Mat& in_img, cv::Mat* out_img,
    const float gaussian_prob, const float size_min, const float size_max, const float sigma)
{
  float prob;
  caffe_rng_uniform(1, 0.f, 1.f, &prob);
  if (prob < gaussian_prob) {
    CHECK_GE(size_max, size_min) << "size max must be >= min.";
    CHECK_GE(size_min, 0) << "size_min must be non-negative";
    float Csize;
    caffe_rng_uniform(1, size_min, size_max, &Csize);
    int Gsize = (int)(Csize);
    if (Gsize % 2 == 0)
      Gsize += 1;
    cv::GaussianBlur(in_img, *out_img, cv::Size(Gsize,Gsize), sigma, sigma);
  } else {
    *out_img = in_img;
  }
}

void pepper(cv::Mat &img, float n)
{
  for(int k=0; k<n; k++)
  {
    int i = rand()%img.cols;
...
void RandomSalt(cv::Mat* out_img, const float salt_prob, const float salt_num)
{
  float prob;
  caffe_rng_uniform(1, 0.f, 1.f, &prob);
  if (prob < salt_prob) {
    CHECK_GE(salt_num, 0) << "size_min must be non-negative";
    float ratio;
    caffe_rng_uniform(1, 0.f, 1.f, &ratio);
    pepper(*out_img, salt_num*ratio);
  }
}
...
cv::Mat ApplyDistort(const cv::Mat& in_img, const DistortionParameter& param) {
  ...
  if (prob > 0.5) {
    ...
    // Do random gaussianBlur
    RandomGaussianBlur(out_img, &out_img, param.gaussian_prob(),
                    param.gaussian_size_min(), param.gaussian_size_max(),
                    param.gaussian_sigma());
                    
    // Do random gaussianBlur
    RandomSalt(&out_img, param.salt_prob(),
                    param.salt_num());
  }
