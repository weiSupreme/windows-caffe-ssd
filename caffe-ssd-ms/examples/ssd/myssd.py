...
train_data = "data/lmdb/VOC2007_trainval_lmdb"
# The database file for testing data. Created by data/VOC0712/create_data.sh
test_data = "data/lmdb/VOC2007_test_lmdb"
...
snapshot_dir = "models/{}".format(job_name)
# Directory which stores the job script and log file.
job_dir = "jobs/{}".format(job_name)
# Directory which stores the detection results.
output_result_dir = "data/output/{}".format(job_name)
...
# job script path.
job_file = "{}.bat".format(model_name)
...
pretrain_model = "data/apple-0.89.caffemodel"
# Stores LabelMapItem.
...
#min_sizes = [min_dim * 10 / 100. - 20] + min_sizes    #conv4_3
#max_sizes = [min_dim * 20 / 100. - 20] + max_sizes
min_sizes = [20, 50]    #conv4_3
max_sizes = [70, 80]
#steps = [8, 16]
...
det_out_param = {
    ...
    'share_location': share_location,
    'background_label_id': background_label_id,
    'nms_param': {'nms_threshold': 0.45, 'top_k': 400},  ########################0.45, 400   Finetuning
    'save_output_param': {
...
with open(job_file, 'w') as f:
  ...
  if solver_param['solver_mode'] == P.Solver.GPU:
    f.write('--gpu {}'.format(gpus, job_dir, model_name))

# Copy the python script to job_dir.
py_file = os.path.abspath(__file__)
shutil.copy(py_file, job_dir)

cmd = 'cmd.exe /k ' + job_file.rstrip('.bat')
os.system(cmd)
